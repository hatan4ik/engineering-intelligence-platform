...
def temporal_tls_config(settings: TemporalWorkerSettings) -> TLSConfig:
    ...
    return TLSConfig(
        server_root_ca_cert=_read_pem(
            settings.temporal_tls_ca_cert_path, "Temporal CA certificate"
        ),
        client_cert=_read_pem(
            settings.temporal_tls_client_cert_path, "Temporal client certificate"
        ),
...
def worker_registration_plan(
    environ: Mapping[str, str] | None = None,
) -> WorkerRegistrationPlan:
...
def build_worker(
    ...
    *,
    ...
    activities: list[ActivityFunction] = []
    ...
    activity_executor = (
        ThreadPoolExecutor(max_workers=8, thread_name_prefix="eip-activity")
        if activities
        else None
    )
    return Worker(
...
async def run_worker() -> None:
    ...
    remediation = (
        _build_remediation_activities() if plan.registration.registered else None
    )
...
def _read_private_key(path: str) -> bytes:
    try:
        ...
        raise RuntimeError(
            "Temporal client private key could not be read from its mounted path"
        ) from exc
    if (
        not content.startswith(b"-----BEGIN ")
        or b"PRIVATE KEY-----" not in content.splitlines()[0]
    ):
