...
| PR Guardian | Shadow E2E reference | GitHub event -> diff/service mapping -> graph/risk -> workflow -> neutral check/comment; principal-scoped, evidence-qualified Decision Context with a GitHub-safe summary; bounded Context Packet and health state; local durable finding/outcome store; signed evidence-read/correction contracts; source-safe check/comment intent persisted before independently recoverable delivery; explicit reviewer-label closure record, canonical feedback-export/report digests; shadow-only onboarding, readiness, bootstrap, and promotion-review contracts | named pilot configuration, managed outbox/receipt storage and recovery operations, externally retained evidence, per-reader authorized explanation view, live reviewer outcomes, independent post-merge correlation, and repository-specific calibration |
