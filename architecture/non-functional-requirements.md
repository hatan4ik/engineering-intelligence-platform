...
| **Durability and recovery** | Workflow state, approvals, jobs, audit records, and externally visible advisory artifacts use managed durable stores in production. An artifact is recorded before delivery, each external effect is independently recoverable/idempotent, and recovery objectives, backup cadence, restore procedure, and regional failure behavior are defined before L3. | Backup/restore drill within RPO/RTO, durable-queue and artifact-delivery restart exercises, audit-export integrity check |
