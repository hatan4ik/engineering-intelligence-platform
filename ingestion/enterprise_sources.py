...
class SlackAdapter:
    ...
    def search(self, *, channel_id: str, latest: str | None = None, limit: int = 100) -> SourcePage:
        if channel_id not in self.policy.allowed_channels:
            return SourcePage(tuple())
            
        params = f"channel={channel_id}&limit={limit}"
        if latest:
            params += f"&latest={latest}"
            
        url = f"{self.base_url.rstrip('/')}/api/conversations.history?{params}"
        headers = {"Authorization": "Bearer REDACTED_FOR_ADAPTER"}
        ...
        for msg in messages:
            ...
            if not self.policy.allows(channel=channel_id, text=text):
                continue
                
            doc = governed_conversation_document(
                provider="slack",
                conversation_id=ts,
                channel=channel_id,
                author=user,
                text=text,
                updated_at=_timestamp(ts),
                revision=ts,
                acl=self.acl,
                source_url=f"slack://channel?id={channel_id}&message={ts}"
            )
            docs.append(doc)
            
        next_cursor = payload.get("response_metadata", {}).get("next_cursor")
        next_url = str(next_cursor) if next_cursor else None
        return SourcePage(tuple(docs), next_url)
