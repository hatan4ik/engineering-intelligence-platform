...
from control_plane.runtime import ControlPlaneConfigurationError, TemporalWorkerSettings
from orchestration.jobs import SqliteJobQueue
from state.audit import SqliteAuditLog
from state.store import SqliteStateStore


def test_temporal_mode_requires_each_evidence_worker_dependency(monkeypatch):
...
def test_temporal_mode_accepts_only_explicit_evidence_worker_configuration(monkeypatch):
    monkeypatch.setenv("EIP_CONTROL_PLANE_MODE", "temporal")
...
def test_temporal_mode_rejects_url_endpoint_or_non_mounted_tls_path(monkeypatch):
    ...
    with pytest.raises(ControlPlaneConfigurationError, match="host:port"):
        TemporalWorkerSettings.from_environment(values)
