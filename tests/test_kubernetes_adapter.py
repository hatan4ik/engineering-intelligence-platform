...
def test_preflight_blocks_unsafe_names_before_mutation():
    ...
    request = ActionRequest(
        "payments;rm -rf /", "prod", "aks.restart.workload", 1, approval_token="x"
    )
    result = execute_control_loop(
        catalog=default_catalog(),
...
def test_failed_verification_without_safe_redo_escalates_instead_of_crashing():
    ...
    request = ActionRequest(
        "payments", "prod", "aks.rollout.undo", 2, approval_token="x"
    )
    result = execute_control_loop(
        catalog=default_catalog(),
        policy=policy(),
...
def test_crashloop_runbook_requires_live_crashloop_evidence():
    ...
    adapter = KubernetesActionAdapter(runner, namespace="prod")
    request = ActionRequest(
        "payments", "prod", "aks.restart.crashloop", 2, approval_token="x"
    )
    result = execute_control_loop(
        catalog=default_catalog(),
        policy=policy(("aks.restart.crashloop",)),
        request=request,
        adapter=adapter,
        approval_verified=True,
    )
...
def test_malformed_kubernetes_payload_denies_before_mutation():
    class MalformedDeploymentRunner(FakeRunner):
        def run(self, argv):
            argv = tuple(argv)
            self.calls.append(argv)
            if "get" in argv and "deployment/payments" in argv:
                return CommandResult(0, '{"spec": {"replicas": "two"}}')
            return CommandResult(0, "ok")

    runner = MalformedDeploymentRunner()
    result = execute_control_loop(
        catalog=default_catalog(),
        policy=policy(),
        request=ActionRequest(
    ...
    assert result.status == "denied"
    assert "KubernetesPayloadError" in (result.error or "")
    assert not any("restart" in call for call in runner.calls)
