...
def test_the_runtime_loader_accepts_a_lapsed_approval_instead_of_crashing(tmp_path):
    """An approval that expired yesterday must not take the publisher down."""
    payload = enforce_payload()
    payload["enforcement"] = {**payload["enforcement"], "expires_on": "2026-08-25"}
    write_config(tmp_path, payload)

    config, error = load_effective_config(tmp_path, repository="acme/platform", now=NOW)

    assert error is None
    assert config.mode is ProductMode.ENFORCE
    assert config.enforcement is not None
    assert config.enforcement.is_active_on(NOW) is False


def test_the_runtime_loader_degrades_to_shadow_and_reports_why(tmp_path):
    path = tmp_path / CONFIG_RELATIVE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("{not json", encoding="utf-8")
...
def test_the_runtime_loader_degrades_to_shadow_on_an_invalid_field(tmp_path):
    write_config(tmp_path, enforce_payload(service_owners=[]))

    config, error = load_effective_config(tmp_path, repository="acme/platform", now=NOW)

    assert config.mode is ProductMode.SHADOW
    assert error is not None and "service_owners" in error


def test_the_runtime_loader_returns_no_error_when_there_is_no_file(tmp_path):
    config, error = load_effective_config(tmp_path, repository="acme/platform", now=NOW)
