...
* **Idempotency.** Each file becomes one event whose id is derived from
  `(repository, ref, path, content)`. The durable SQLite ledger dedupes, so re-running over an
  unchanged checkout writes nothing and reports `documents: 0` with `duplicates` equal to the
  file count.
...
| `AZURE_SEARCH_API_KEY` **or** `AZURE_CLIENT_ID` | The credential. An API key builds a key credential; `AZURE_CLIENT_ID` selects the workload identity that `DefaultAzureCredential` resolves, which is how the deployed platform runs |
